...
{ 
    ...
    /**读取输入矩阵.*/
    Tensor* q =new Tensor(std::vector<size_t>{BatchSize,(*InputShape)[0],(*InputShape)[1]}, "CPU", 0);
    q->FillArray(3.);
    CGNode *tuq = new CGNode(q, 1);
    /**初始化参数矩阵.*/
    Tensor* w =new Tensor(std::vector<size_t>{BatchSize,(*OutputShape)[0],(*InputShape)[0]}, "CPU", 0);
    w->FillArray(5.);
    CGNode *tuw = new CGNode(w, 1);
    Tensor* e =new Tensor(std::vector<size_t>{BatchSize,(*InputShape)[1],(*OutputShape)[1]}, "CPU", 0);
    e->FillArray(7.);
    ...
    Tensor* LabelTensor =new Tensor(std::vector<size_t>{BatchSize,(*OutputShape)[0],(*OutputShape)[1]}, "CPU", 0);
