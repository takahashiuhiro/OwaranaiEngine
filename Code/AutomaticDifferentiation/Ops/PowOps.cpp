...
void PowOps::Backward()
{
    ...
    if(this->CG->GetNode(NodeidList[0])->Property.Get<bool>("RequireGrad"))
    {
        ...
        std::string MulNode = this->CG->GetNodeidByOps(OpsType::EleMul, {ThisDNodeid, Nodeid});
        this->CG->RegisterVariableNode(MulNode);
        this->CG->RegisterOpsCompleted(MulNode, {ThisDNodeid, PowNode}, OpsType::EleMul, Dict());
        this->CG->GetCGOps(MulNode)->AfterSettingShapeComputing();

        std::string NewDNode = this->CG->GetDPartNodeid(NodeidList[0], Nodeid);
        this->CG->RegisterVariableNode(NewDNode);
        this->CG->RegisterOpsCompleted(NewDNode, {MulNode}, OpsType::Add, Dict());
        this->CG->GetCGOps(NewDNode)->SetAddWeight({{MulNode, GetEleExponent()}});
